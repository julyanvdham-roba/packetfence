package pf::Switch::Ubiquiti::Unifi9;

=head1 NAME

pf::Switch::Ubiquiti::Unifi9

=head1 SYNOPSIS

The pf::Switch::Ubiquiti::Unifi9 module implements an object oriented interface to
...
sub description { 'Unifi 9.1.120+ Controller' }
...
Parse external portal request using URI and its parameters then return an hash reference with the appropriate parameters
...
sub _connect {
    ...
    $ua->default_header('X-API-TOKEN' => $password);

    my $base_url = "$transport://$controllerIp/proxy/network/integration/v1";

    $response = $ua->get($baseUrl."/info");
    unless($response->is_success) {
        $logger->error("Unable to authenticate against Unifi API: ".$response->status_line);
        die;
    }
...
sub _deauthenticateMacWithHTTP {
    ...
    my $response = $ua->get("$base_url/sites");

    unless($response->is_success) {
        $logger->error("Unable to obtain site list from the Unifi controller: ".$response->status_line);
        return;
    }
    ...
    foreach my $entry (@{$sites->{'data'}}) {
        $response = $ua->get("$base_url/sites/$entry->{'id'}/clients?filter=macAddress.eq('$mac')");
        
        if ($response->is_success) {
            my $clients = decode_json($response->decoded_content());
            if ($clients->{'count'} gt 0) {
                $found = $TRUE;
                $clientID = $clients->{'data'}[0]{'id'};
                $site_opts{'id'} = $entry->{'id'};
                $site_opts{'name'} = $entry->{'name'};
                $site_opts{'internalReference'} = $entry->{'internalReference'};
                $logger->info("Found client on site: $site_opts{'name'} | Client ID: $clientID");
                last;
            }
        }
    }
    ...
    if ($found) {
        if ($command eq "authorize-guest") {
            $response = $ua->post("$base_url/sites/$site_opts{'id'}/clients/$clientID/actions", Content => encode_json({
                action => "AUTHORIZE_GUEST_ACCESS",
                timeLimitMinutes => $args->{minutes}
            }));

            if ($response->is_success) {
                $logger->trace("Successfully authorized guest on site: $site_opts{'name'}");
            } else {
                $logger->error("Unable to authorize guest: " .$response->status_line);
            }
        } else if ($command eq "unauthorize-guest") {
            $response = $ua->post("$base_url/sites/$site_opts{'id'}/clients/$clientID/actions", Content => encode_json({
                action => "UNAUTHORIZE_GUEST_ACCESS"
            }));

            if ($response->is_success) {
                $logger->trace("Successfully unauthorized guest on site: $site_opts{'name'}");
            } else {
                $logger->error("Unable to unauthorize guest: " .$response->status_line);
            }
        }
    }
...
sub populateAccessPointMACIP {
    ...
    unless($response->is_success) {
        $logger->error("Unable to obtain the site list from the Unifi controller: ".$response->status_line);
        return;
    }
    ...
    foreach my $entry (@{$json_data->{'data'}}) {
        $response = $ua->get("$base_url/sites/$entry->{'id'}/devices");
        unless($response->is_success) {
            $logger->error("Unable to obtain the device list from the Unifi controller: ".$response->status_line);
            return;
        }
        my $json_data_ap = decode_json($response->decoded_content());
        foreach my $AP (@{$json_data_ap->{'data'}}) {
            $cache->set("Ubiquiti-" . $AP->{'macAddress'} , $AP->{'ipAddress'} );
        }
    }
}
...
Copyright (C) 2025 Roba Metals B.V.
